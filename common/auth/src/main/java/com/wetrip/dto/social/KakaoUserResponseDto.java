...
public class KakaoUserResponseDto { // 카카오 로그인 - 이중 구조
    ...
    public static KakaoUserResponseDto from(Map<String, Object> attributes) {
        ...
        return KakaoUserResponseDto.builder()
            ...
            .email(
                SocialResponseUtil.safeString(kakaoAccount != null ? kakaoAccount.get("email") : null))
            .name(
                SocialResponseUtil.safeStringOrDefault(profile != null ? profile.get("nickname") : null,
                    "카카오사용자")) // 카카오는 이름 제공 X - 닉네임으로 받아옴
            .profileImageUrl(SocialResponseUtil.safeString(
                profile != null ? profile.get("profile_image_url") : null))
            .ageRange(SocialResponseUtil.safeString(
                kakaoAccount != null ? kakaoAccount.get("age_range") : null))
            .gender(
                SocialResponseUtil.safeString(kakaoAccount != null ? kakaoAccount.get("gender") : null))
            .contact(SocialResponseUtil.safeString(
                kakaoAccount != null ? kakaoAccount.get("phone_number") : null))
            .birthday(SocialResponseUtil.safeString(
                kakaoAccount != null ? kakaoAccount.get("birthday") : null))
            .birthyear(SocialResponseUtil.safeString(
                kakaoAccount != null ? kakaoAccount.get("birthyear") : null))
            .build();
    }
